...
function SaleForm({ onSaleAdded }) {
  ...
  // Fetch customer accounts when payment type is CREDIT
  useEffect(() => {
            ...
            <div className="relative">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                {paymentType === "CARD" ? (
                  ...
                  <UserCheck className="h-5 w-5 text-[#1C3333]/70" />
                ) : (
          ...
          {paymentType === "CREDIT" && (
            <div className="space-y-2 md:col-span-2">
              ...
              <select
                ...
                onChange={(e) => {
                  setCustomerAccountId(e.target.value);
                  const selectedAccount = customerAccounts.find(acc => acc.id === Number(e.target.value));
                  if (selectedAccount) {
                    setCustomerName(selectedAccount.name);
                    setCustomerEmail(selectedAccount.contactEmail || "");
                    setCustomerPhone(selectedAccount.contactPhone || "");
                  }
                }}
                className="w-full px-3 py-2 border cursor-pointer border-[#1C3333]/30 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#1C3333] focus:border-[#1C3333] text-[#1C3333] bg-white"
                required={paymentType === "CREDIT"}
              >
                <option value="">Select customer...</option>
                {customerAccounts.map((account) => (
                  <option key={account.id} value={account.id}>
                    {account.name} {account.balance !== 0 && `(Balance: Rs.${account.balance.toFixed(2)})`}
                  </option>
                ))}
              </select>
              {customerAccounts.length === 0 && (
                <p className="text-sm text-[#1C3333]/70">
                  No customer accounts found. Create one in the Ledger section.
                </p>
              )}
            </div>
          )}
